...
def _Connectivity(inputfile):

  print("=========================================")
  print("       Building connetivity")
  print("=========================================\n\n")
  ...
  # downstream
  for j in tqdm(range(numelements), desc = "Reading elements"):
  ...
  for k1 in tqdm(range(numelements), desc = "Building network"):
...
def _TrackUpstream(contents, connectivity):  # new name: connectivity
  ...
  if not os.path.isfile('drainnetwork_sid.bin'):
    ...
    for j in tqdm(range(len(idsistecol)), desc = "Computing drainage area"):
...
def _RelativeImportance(contents, drainArea, connectRecno):
  ...
  for i in tqdm(range(numcatchments), desc = "Computing relative importance"):
    catchmentRI = 0.
  ...
  print()
  print()
  print("\n\nFinished. Results stored in the file Priorities.txt\n\n")
  print()
  print()



def launch(inputfile):
  df, conectividad, conectividadRecno = _Connectivity(inputfile)
  BasinArea = _TrackUpstream(df, conectividad)
  _RelativeImportance(df, BasinArea, conectividadRecno)
